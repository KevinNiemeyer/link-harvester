...
(function($) {
        ...
        for (key1 in retObj) {
            ...
            if (key1 === "emailAddresses") {
                ...
                for (var i = 0; i < retObj[key1].length; i++) {
                    var link = $('<li style="list-style-type: none;">').html(`${i+1}: ${retObj[key1][i]}`);
                    header.append(link);
                    $("#result").append(header);
                }
            } else if (key1 === "links") {
                var header = $('<div style="margin: 20px;"><ol>').html("<h2>Links:</h2>");
                for (var i = 0; i < retObj["links"].length; i++) {
                    var header2 = $('<li style="list-style-type: none;">').html(`${i+1}: ${retObj["links"][i]["linkText"]}: ${retObj["links"][i]["url"]}`);
    ...
    function harvestEmails(textBlock) {
        ...
        let emailsPattern = /mailto:([a-zA-Z0-9_\-\.]+@[a-zA-Z0-9_\-\.]+\.[a-zA-Z]{2,5})/ig,
            emailsArray = [],
            execArray = [];
